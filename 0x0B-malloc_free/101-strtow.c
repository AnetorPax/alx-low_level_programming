...
/**
 * allocate_memory - Aollcates memory to a function
...
char **strtow(char *str)
{
	...
	for (i = 0; i < word_count; i++)
	{
		...
		lent = get_word_length(str);
		words[i] = malloc((lent + 1) * sizeof(char));
		if (words[i] == NULL)
		{
			free_memory(words, i);
			return (NULL);
